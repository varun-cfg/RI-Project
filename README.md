...
> Note: This repository includes an extended LIBERO evaluator with optional action re-sampling, jerk/CVR metrics, and a constraint-aware policy wrapper. See ../../README_.md for details.

Quick start (constraints ON, center-crop, limited trials)
```bash
python experiments/robot/libero/run_libero_eval.py \
  --pretrained_checkpoint openvla/openvla-7b-finetuned-libero-spatial \
  --task_suite_name libero_spatial \
...
Optional: enable re-sampling (adds extra model calls per step)
```bash
python experiments/robot/libero/run_libero_eval.py \
  --pretrained_checkpoint openvla/openvla-7b-finetuned-libero-spatial \
  --task_suite_name libero_spatial \
